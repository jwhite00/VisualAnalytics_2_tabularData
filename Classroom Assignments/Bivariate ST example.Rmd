...
This time, I will use the [data about mydata4](https://data.seattle.gov/Public-Safety/mydata4-Data/4fs7-3vj5) from the Seattle Open Data portal:
...
```{r str, eval=FALSE}
str(mydata4,width = 70,strict.width='cut')
```
...
library(magrittr)
(mydata4Total=table(mydata4$access_mode,mydata4$timeperiod)%>% #create relative values table and then...
...
Those tables show total counts or percents. However, when a table tries to hypothesize a relationship, you should have the _independent_ variable in the columns, and the _dependent_ one in the rows; then, 


```{r table_byCol,eval=FALSE}
mydata4Col=table(mydata4$access_mode,mydata4$timeperiod)%>% 
         prop.table(margin = 2)%>%   # 2 is % by column
         "*"(100)%>%
         round(3)

#the percent should be calculated by column, to see how the levels of the dependent variable varies by each level of the independent one, and compare along rows. W/ indep into columns, dependent into rows
...
df.T=as.data.frame(mydata4Total) # table of proportion based on total
# YOU GET:
head(df.T)
```
...
```{r, eval=FALSE}
names(df.T)=c('TravelMode','TimeofDay','Percent') #renaming
head(df.T)
```
...
library(ggplot2)                           
base = ggplot(df.T, aes(TimeofDay, TravelMode)) 
...
...some more work:
```{r, eval=FALSE}
tablePlot3 = tablePlot2 + scale_size_continuous(range=c(0,10)) #change 10? this changes the size and makes the large numbers much bigger 
...
```{r facet, eval=FALSE}
base  = ggplot(df.T, aes(x = TravelMode, y = Percent ) ) 
bars1 = base + geom_bar( stat = "identity" ) + theme_minimal()
# bar per day time with 'facet'
bars2 = bars1 + facet_wrap( ~ TimeofDay ,nrow = 1) 
...
df.C=as.data.frame(mydata4Col)
colnames(df.C)=c('TravelMode','TimeofDay','Percent')
#####

base  = ggplot(df.C, aes(x = TravelMode, y = Percent ) ) 
bars1 = base + geom_bar( stat = "identity" )
bars2 = bars1 + facet_wrap( ~ TimeofDay ,nrow = 1) 
bars2 + coord_flip()
```

The type of mydata4 is not ordinal, then we could reorder the bars:

```{r orderFacet, eval=FALSE}
base  = ggplot(df.C, aes(x = reorder(TravelMode, Percent), y = Percent ) ) 
bars1 = base + geom_bar( stat = "identity" )
bars2 = bars1 + facet_wrap( ~ TimeofDay ,nrow = 1) 
...
# heatplot
base  = ggplot(df.C, aes(x = TimeofDay, y = reorder(TravelMode, Percent), fill = Percent)) 
...
In the previous data set we had a variable that informs the amount of days it takes someone to report a mydata4:

```{r summaryTime, eval=FALSE}
summary(mydata4$DaysToReport)
```
...
```{r tapply, eval=FALSE}
tapply(mydata4$DaysToReport,mydata4$Precinct,mean)
```


Above, you see the mean time (in days) it takes per precint for people to notify a mydata4. You can suddenly create a plot in your mind just by reading those values, but the plot you imagine may be far from this one:

```{r,boxNumCat1, eval=FALSE}
boxplot(DaysToReport~Precinct,data=mydata4)
```
...
tapply(mydata4$DaysToReport,mydata4$Precinct,summary)
```

From the information above, you know that for each precint, the 75% of mydata4s are reported in a day or less. If we consider that situation as the expected behavior, we could omit those cases:
...
boxplot(DaysToReport~Precinct,data=mydata4,subset = DaysToReport>=7,
        main="One week or more")

boxplot(DaysToReport~Precinct,data=mydata4,subset = DaysToReport>=30,
        main="One 30-day month or more")

boxplot(DaysToReport~Precinct,data=mydata4,subset = DaysToReport>=180,
        main="180 days or more")

boxplot(DaysToReport~Precinct,data=mydata4,subset = DaysToReport>=365,
        main="One year or more")
```

Up to this point, you need to be planing a good _story_. The situation is different for each case, but let's build our story from the mydata4s that take a year or longer to report.

First, let's see how many cases we have per precinct:

```{r mydata4Tapply, eval=FALSE}
mydata4Year=mydata4[mydata4$DaysToReport>=365,]
tapply(mydata4Year$DaysToReport,mydata4Year$Precinct,length)
```

The year the mydata4 occurred would be another variable to consider, to see if we can filter more cases:

```{r casesTapplycount, eval=FALSE}
tapply(mydata4Year$DaysToReport,mydata4Year$year,length)
```
...
```{r mydata4After2000, eval=FALSE}
mydata4Y2000=mydata4[(mydata4$DaysToReport>=365) & (mydata4$year>=2000),]
tapply(mydata4Y2000$DaysToReport,mydata4Y2000$Precinct,length)
```

Now, we see a better boxplot:
```{r boxpAfter2000, eval=FALSE}
boxplot(DaysToReport~Precinct,data=mydata4Y2000,
...
mydata4Y2000$YearsToReport=mydata4Y2000$DaysToReport/365
boxplot(YearsToReport~Precinct,data=mydata4Y2000,
        main="One year or more (from 2000)")
```
...
boxplot(YearsToReport~Precinct,data=mydata4Y2000,subset = YearsToReport>=5,
...
Let's try _weekday_ and _Ocurred.Time of Day_:
...
boxplot(YearsToReport~weekday,data=mydata4Y2000,
        main="One year or more BY WEEKDAY (from 2000)",las=2)

boxplot(YearsToReport~access_mode,data=mydata4Y2000,
        main="One year or more BY TIME mydata4 OCCURRED (from 2000)",las=2)

```

Not much variety is perceived; then let's try _timeperiod_ and _year_:

```{r exploreBOX2, eval=FALSE}
par(mfrow=c(2,1))

boxplot(YearsToReport~year,data=mydata4Y2000,
        main="One year or more (from 2000)",las=2)

boxplot(YearsToReport~timeperiod,data=mydata4Y2000,
        main="One year or more (from 2000)",las=2)
```
...
# no missing:
mydata4YearGG=mydata4Y2000[complete.cases(mydata4Y2000$YearsToReport),]

base = ggplot(mydata4YearGG,aes(x=factor(year), y=YearsToReport)) 
...
Now, let's complete the story by telling how the data filtered behaves, that is, the mydata4s that took less than a year to report since 2000 (we will not include data from before):

```{r mydata4Week, eval=FALSE}
# data preparation:

mydata4Week=mydata4[(mydata4$DaysToReport<365) & (mydata4$year>=2000),]

mydata4Week$WeeksToReport=mydata4Week$DaysToReport/7

mydata4YearGG2=mydata4Week[complete.cases(mydata4Week$WeeksToReport) &complete.cases(mydata4Week$timeperiod),]


```

```{r plotmydata4Week, eval=FALSE}
#plotting it:
base = ggplot(mydata4YearGG2,aes(x=factor(year), y=WeeksToReport)) 
...
We also found variability in the type of mydata4, so we could try a story with it; first with Years to report (for mydata4s that took a year or longer to report, after year 2000):

```{r bymydata4, eval=FALSE}
base= ggplot(mydata4YearGG,
             aes(x = reorder(timeperiod, YearsToReport, FUN = max), # reorder!
                 y=YearsToReport)) 
mins = base + stat_summary(fun.y=min,size=1,
                           geom="line", linetype='dashed',show.legend = T,
                           aes(group=1,col='Min'))
...
Now, for mydata4s that took less than  year to report after year 2000:

```{r bymydata42, eval=FALSE}
base = ggplot(mydata4YearGG2,
              aes(x = reorder(timeperiod, WeeksToReport, FUN = max),
                  y=WeeksToReport)) 
mins = base + stat_summary(fun.y=min,size=1,
                           geom="line", linetype='dashed',show.legend = T,
                           aes(group=1,col='Min'))
...
```{r anova, eval=FALSE}
#making a subset:
anovaData=mydata4Y2000[mydata4Y2000$YearsToReport>=5,]
...
# 1. MEAN of days it takes to report a mydata4 by neighborhood
daysByNeigh=tapply(mydata4$DaysToReport, mydata4$Neighborhood, mean,na.rm=T)

# you have:
head(daysByNeigh)
```

* Aggregating mydata4s by neighborhood
```{r, eval=FALSE}
# 2. PROPORTION of mydata4s by neighborhood
mydata4sByNeigh=tapply(mydata4$timeperiod, mydata4$Neighborhood, length)%>%      
                     prop.table()%>%
                     "*"(100)%>% 
                     round(2) 
head(mydata4sByNeigh)
```
...
mydata4sByNeigh=as.data.frame(mydata4sByNeigh)%>%rownames_to_column()
```
...
num_num=merge(daysByNeigh,mydata4sByNeigh) # 'row name' is the "key"
head(num_num)
```
...
base = ggplot(num_num, aes(daysByNeigh,mydata4sByNeigh)) 
...
We can limit the labels, annotating the ones that represent at least 5% of the mydata4s in the city:

```{r, eval=FALSE}
base = ggplot(num_num, aes(daysByNeigh,mydata4sByNeigh,label=rowname)) 
plot1= base +  geom_point() 
plot1 + geom_text_repel(aes(label=ifelse(mydata4sByNeigh>=5,
                                         num_num$rowname, "")))
```
...
scatp1 + geom_text(aes(label=ifelse(mydata4sByNeigh>=5,num_num$rowname, "")))
```
...
scatp2= scatp1 + geom_text_repel(aes(label=ifelse(mydata4sByNeigh>=5,
                                                  num_num$rowname,
                                                  ""))) 
